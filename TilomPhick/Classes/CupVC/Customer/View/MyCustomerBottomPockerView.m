...
@implementation MyCustomerBottomPockerView
-(instancetype)initWithController:(UIViewController *)controller{
    ...
    if(self){
        ...
        //LHEditPickerView
        MyCustomerPickerView *editView=[[MyCustomerPickerView alloc]initWithFrame:CGRectMake(0, controller.view.bounds.size.height, controller.view.bounds.size.width, controller.view.bounds.size.height/5*2)];
...
+(instancetype)showWithController:(UIViewController *)controller andData:(NSArray *)data{
    MyCustomerBottomPockerView *bottom=[[self alloc]initWithController:controller];
...
+(void)showEditPickerViewWithController:(UIViewController *)controller andData:(NSArray *)data andBlock:(void (^)(NSString *temp))bottomResultPresent{
    MyCustomerBottomPockerView *bottom=[MyCustomerBottomPockerView showWithController:controller andData:data];
