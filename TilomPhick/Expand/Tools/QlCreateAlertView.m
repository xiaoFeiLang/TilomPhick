...
static QlCreateAlertView *singCreateAlertView =nil;

@implementation QlCreateAlertView
#pragma mark--单例
+(QlCreateAlertView *)shareSingleCreateAlertView{
    
    static dispatch_once_t once;
    dispatch_once(&once, ^{
        singCreateAlertView=[[QlCreateAlertView allocWithZone:NULL]init];
    });
...
-(instancetype)initWithNameArray:(NSArray *)nameArray andMenuOrigin:(CGPoint)orign andMenuWidth:(CGFloat)width andHeight:(CGFloat)rowHeight andLayer:(CGFloat)layer andTableViewBackGroundColor:(UIColor *)color andIsSharp:(BOOL)sharp andType:(popType)poptype{
    ...
    if (self) {
        ...
        for (NSInteger i=0; i<nameArray.count; i++) {
            UIView *view=[[UIView alloc]initWithFrame:CGRectMake(0, (i+1)*rowHeight - 2, self.menuTableView.frame.size.width, 1)];
            [view setBackgroundColor:RGBHEX(0xEEEEEE)];
...
-(instancetype)initWithNameArray:(NSArray *)nameArray andMenuOrigin:(CGPoint)orign andMenuWidth:(CGFloat)width andHeight:(CGFloat)rowHeight andLayer:(CGFloat)layer andTableViewBackGroundColor:(UIColor *)color andIsSharp:(BOOL)sharp andType:(popType)poptype redrawSharp:(RedrawSharp)redrawSharp{
    
    self=[super initWithFrame:CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGHT) ];
    if (self) {
        
        UIImage *img = LOAD_IMAGE(@"left-bg");
//        width = img.size.width;
        ...
        img = [img resizableImageWithCapInsets:UIEdgeInsetsMake(30, 0, 32, 0)];
        self.bgImgView = [[UIImageView alloc]initWithImage:img];
        CGFloat y = self.menuTableView.top - RC_Y(8) - 0.5;
        self.bgImgView.frame = CGRectMake(self.menuTableView.left, y, self.menuTableView.width, self.menuTableView.height + RC_Y(8));
...
-(UIView *)createAlertWithTitle:(NSString *)title andOkButtonTitle:(NSString *)okButtonTitle andCancelButtonTitle:(NSString *)cancelButtonTitle andAimTitle:(NSString *)aimTitle andAlertBody:(NSString *)body andIsTextField:(BOOL)isHas andViewLayer:(CGFloat)layer{
    ...
    UIView *view=[[UIView alloc]initWithFrame:CGRectMake((SCREENWIDTH-BUTTONALERTVIEWWIDTH)/2,( self.view.frame.size.height-BUTTONALERTVIEWHEIGHT)/2-44, BUTTONALERTVIEWWIDTH, BUTTONALERTVIEWHEIGHT)];
    ...
    if (!isHas) {
        ...
        [QlCreateAlertView shareSingleCreateAlertView].textField=textfiled;;
...
#pragma mark--消失事件
-(void)dismissWithCompletion:(void (^)(QlCreateAlertView *))completion{
    __weak typeof(self)weakSelf=self;
    [UIView animateWithDuration:0.1 animations:^{
        weakSelf.menuTableView.frame=CGRectMake(_orign.x, _orign.y, _width, 0);
        weakSelf.bgImgView.frame = CGRectMake(_orign.x, _orign.y, _width, 0);
