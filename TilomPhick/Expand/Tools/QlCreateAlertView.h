...
@class QlCreateAlertView;

typedef void (^Dismiss)(void);
typedef UIView *(^RedrawSharp)(UITableView *, QlCreateAlertView *);
typedef enum : NSInteger{
    kNavigationPop=1000,
    ksortPop
}popType;
@protocol ButtonClick;
@protocol QlCreatePopViewDelegate;
@interface QlCreateAlertView : UIView<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
@property(nonatomic,strong)UILabel *titleLabel;
@property(nonatomic,assign)id<ButtonClick>delegate;
@property(nonatomic,strong)id<QlCreatePopViewDelegate>tableViewDelegate;
...
//下拉菜单消失事件
-(void)dismissWithCompletion:(void (^)(QlCreateAlertView *create))completion;
...
@protocol QlCreatePopViewDelegate <NSObject>
